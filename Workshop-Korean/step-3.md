...
메타 데이터를 추출하고 확인한 후 이제 상태 시스템에 몇 가지 단계를 추가할 준비가되었습니다. 엄지 손톱 찍기, 이미지 인식 및 메타 데이터 색인 유지 / 영구화입니다. 엄지 손톱과 이미지 인식은 서로 의존하지 않고 병렬로 발생할 수 있으며 [평행 상태](https://docs.aws.amazon.com/step-functions/latest/dg/amazon-states-language-parallel-state.html)을 참조하십시오.

<img src="images/3-state-machine-parallel.png" width="60%">

### 3A 단계 : 상태 머신 정의 업데이트

이제 상태 시스템에 단계를 점진적으로 추가, 배치 및 테스트하고, 현재 상태 시스템에 병렬 단계를 추가하는 몇 가지 경험을 얻었습니다 (마지막 상태의 최종 JSON에서 내보내거나 가져 오는 것을 잊지 마십시오). )을 사용하여 엄지 손톱과 이미지 인식을 동시에 수행 할 수 있습니다.

구문에 대한 도움이 필요하면 다음 설명서를 살펴보십시오.
...
- 각 분기에 대해 해당 람다 함수를 트리거하는**Task**상태가있는 상태 머신 개체를 만듭니다.
...
이제 단계 1D를 기억할 수있는 좋은 시간이었습니다. 상태 입력을 병합하여 출력과 병합하는 방법을 배웠으므로 [AWS 단계 함수 경로]기능(https://docs.aws.amazon.com/step-functions/latest/dg/awl-ref-paths.html)).

특히 *Parallel* 상태에서**ResultPath**를 사용하여 병렬 상태 시스템 출력의 배열을 다운 스트림 상태에서 사용할 수있게하십시오. `parallelResults` 속성을 사용하십시오.
...
이제 원하는 이미지로 상태 머신을 테스트하십시오!

- Amazon Rekognition에서 감지 한 레이블과 축소판의 S3 위치가 모두 출력에서 ​​사용 가능한지 확인하십시오.

	<img src="images/3-state-machine-parallel-with-output.png" width="90%">

- 축소판이**Thumbnail**폴더 아래의 S3 버킷에 있는지 확인하십시오.
...
### 다음 단계
이제 [Step 4](step-4.md)로 이동할 준비가되었습니다!
