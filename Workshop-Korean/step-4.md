...
이 시점에서 우리는 상태 머신이 섬네일과 메타 데이터를 생성 할뿐만 아니라 이미지 (레이블)에 어떤 객체와 개념이 있는지 탐지해야합니다.


### 4A 단계 : DynamoDB에 메타 데이터를 저장하는 단계 추가

이 부분에서는 다시 JSON을 수동으로 편집 할 것입니다. 이전에 사용한 편집기로 이동하여 최종 JSON에 붙여 넣으면 3 단계에서 끝납니다. 여기에서 DynamoDB에 모든 것을 저장하는 람다 함수 `sfn-workshop-setup-PersistDDB`를 실행하기 위해 상태 시스템에 한 단계 더 추가하십시오.
